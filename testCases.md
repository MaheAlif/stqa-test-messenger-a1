...
| TC-AUTH-003 | Signup with Empty Username             | None                                    | POST /auth/signup (empty username) | 400 Bad Request                      | 201 Created (userId 102)             | Completed | TC-AUTH-003                              | Pass      |
| TC-AUTH-004 | Signup with Empty Email                | User "alice" exists                     | POST /auth/signup (empty email)    | 400 Bad Request "Email required"     | 400 Bad Request "Username taken"     | Completed | TC-AUTH-004                              | **Fail**  |
| TC-AUTH-005 | Signup with Invalid Email Format       | None                                    | POST /auth/signup (invalid email)  | 400 Bad Request                      | 201 Created (userId 103)             | Completed | TC-AUTH-005                              | **Fail**  |
| TC-AUTH-006 | Signup with Empty Password             | None                                    | POST /auth/signup (empty password) | 400 Bad Request                      | 201 Created (userId 104)             | Completed | TC-AUTH-006                              | **Fail**  |
| TC-AUTH-007 | Valid Login                            | User "alice" exists                     | POST /auth/login with credentials  | 200 OK with JWT token                | 200 OK with JWT token                | Completed | TC-AUTH-007                              | Pass      |
| TC-AUTH-008 | Login with Wrong Password              | User "alice" exists                     | POST /auth/login (wrong password)  | 401 Unauthorized                     | 401 Unauthorized                     | Completed | TC-AUTH-008                              | Pass      |
| TC-AUTH-009 | Login with Non-existent User           | User "nobody" does not exist            | POST /auth/login (invalid user)    | 401 Unauthorized                     | 401 Unauthorized                     | Completed | TC-AUTH-009                              | Pass      |
| TC-AUTH-010 | Logout (Valid User)                    | User "alice" logged in                  | POST /auth/logout?username=alice   | 200 OK                               | 200 OK                               | Completed | TC-AUTH-010                              | Pass      |
| TC-AUTH-011 | Auth Test with Valid Token             | User logged in with valid token         | GET /auth/test with valid token    | 200 OK                               | 200 OK                               | Completed | TC-AUTH-011                              | Pass      |
| TC-AUTH-012 | Auth Test with Invalid Token           | None                                    | GET /auth/test with invalid token  | 403 Forbidden                        | 403 Forbidden                        | Completed | TC-AUTH-012                              | Pass      |
| TC-AUTH-013 | Auth Test without Token                | None                                    | GET /auth/test without token       | 403 Forbidden                        | 403 Forbidden                        | Completed | TC-AUTH-013                              | Pass      |
...
| TC-AUTH-015 | Signup with SQL Injection              | None                                    | POST /auth/signup (username: ' OR '1'='1) | 400 Bad Request                  | 201 Created (userId 106)             | Completed | TC-AUTH-015                              | **Fail**  |
| TC-AUTH-016 | Signup with Duplicate Email            | Email alice@test.com exists             | POST /auth/signup (different username) | 400 Bad Request                      | 201 Created (userId 107)             | Completed | TC-AUTH-016                              | **Fail**  |
| TC-AUTH-017 | Login without Password                 | User "Fariha" exists                    | POST /auth/login (empty password)  | 401 Unauthorized                     | 200 OK with JWT token                | Completed | TC-AUTH-017                              | **Fail**  |
| TC-AUTH-018 | Login with Email Only                  | 2 users with same email exist           | POST /auth/login (only email)      | 400 Bad Request                      | 500 Internal Server Error            | Completed | TC-AUTH-018                              | **Fail**  |
| TC-AUTH-019 | Login with Missing Username Field      | None                                    | POST /auth/login (no username)     | 500 Internal Server Error            | 500 Internal Server Error            | Completed | TC-AUTH-019                              | Pass      |
| TC-AUTH-020 | Login with Wrong Email (Ignored)       | User "alice" exists                     | POST /auth/login (wrong email)     | 400 Bad Request                      | 200 OK with JWT token                | Completed | TC-AUTH-020                              | **Fail**  |
| TC-AUTH-021 | Logout with Non-Existent User          | User "ghost" does not exist             | POST /auth/logout?username=ghost   | 404 Not Found                        | 200 OK                               | Completed | TC-AUTH-021                              | **Fail**  |
| TC-AUTH-022 | Logout with Empty Username             | None                                    | POST /auth/logout?username=        | 400 Bad Request                      | 200 OK                               | Completed | TC-AUTH-022                              | **Fail**  |
| TC-AUTH-023 | Logout with Invalid Username Format    | None                                    | POST /auth/logout?username=@@@###  | 400 Bad Request                      | 200 OK                               | Completed | TC-AUTH-023                              | **Fail**  |
| TC-AUTH-024 | Logout with Extra Parameter            | User "alice" logged in                  | POST /auth/logout (username + role) | 400 Bad Request                     | 200 OK                               | Completed | TC-AUTH-024                              | **Fail**  |
| TC-AUTH-025 | Login with SQL Injection               | None                                    | POST /auth/login (SQL in username) | 401 Unauthorized                     | 401 Unauthorized                     | Completed | TC-AUTH-025                              | Pass      |
| TC-AUTH-026 | Login with Empty Body                  | None                                    | POST /auth/login with {}           | 400 Bad Request                      | 400 Bad Request                      | Completed | TC-AUTH-026                              | Pass      |
